...
*/
namespace KotaVillage\Database;

use KotaVillage\Util;
...
(<a href="./nojsstatus" target="kotavillagestatus">Hotspot Status nojs</a>), or the preferred javascript version
(<a href="javascript: loginwindow = window.open('http://10.1.0.1/kotavillage/uam/mini', 'kotavillagestatus',
...
EOT
        ...
        if ($this->Settings->getTemplate('loggedinnojshtml') === null) {
            $this->Settings->setTemplate(
                'loggedinnojshtml',
                <<<'EOT'
<p>Your login was successful. Please click <a href="nojsstatus" target="kotavillagestatus">HERE</a> to open a status window
...
EOT
    ...
    private function migrateGroups()
    {
        ...
        foreach ($groups as $group => $expiry) {
            $attributes = array();
            $attributes['GroupName'] = \KotaVillage\Clean::groupName($group);

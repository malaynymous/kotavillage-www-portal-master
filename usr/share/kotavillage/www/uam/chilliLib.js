...
chilliController.processReply = function ( resp ) {
	...
	if (  (typeof ( resp.session ) == 'object') &&
		...
		{
    	    loginwindow = window.open("/kotavillage/uam/mini", "kotavillagestatus", "width=300,height=400,location=no,directories=no,status=yes,menubar=no,toolbar=no");
    	    if(loginwindow)
    	    {
...
function updateUI (cmd ) {
	...
	if (chilliController.redir.originalURL != null &&
	    ...
	    $('#userurl').click(function(){
    	    loginwindow = window.open("/kotavillage/uam/mini", "kotavillagestatus", "width=300,height=500,location=no,directories=no,status=yes,menubar=no,toolbar=no");
    	    loginwindow.moveTo(100,100);
    	    loginwindow.focus();
    	    //return false;
	    });

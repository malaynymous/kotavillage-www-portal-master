...
{
    // Set cookie
    setcookie('kotavillagenojs','javascriptdisabled', time()+60*60*24*30);
...
{
    // Set cookie
    setcookie('kotavillagenojs','', time()-60*60*24*30);
    // Redirect via header to reload page?
    header("Location: http://$lanIP:3990/prelogin");
}
...
}elseif( isset($_COOKIE['kotavillagenojs']) && $_COOKIE['kotavillagenojs'] == 'javascriptdisabled')
{
    $nojs = true;
    $smarty->assign("nojs" , true);
    $smarty->assign("js" , false);
...
/*$jsloginlink = "http://$lanIP/kotavillage/uam/mini?$query";

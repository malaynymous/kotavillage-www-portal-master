...
{
	$resourcefiles = array(
	    '/usr/share/kotavillage/www/hotspot.css',
	    '/usr/share/kotavillage/www/radmin/radmin.css',
	    '/usr/share/kotavillage/www/js/kotavillage.js',
	    '/usr/share/kotavillage/www/radmin/js/radmin.js'	    
	    );
...
function bandwidth_options()
{
    ...
    foreach($kbits_options as $kbits)
    {
        ...
        $label = \KotaVillage\Util::formatBits($bits) ." ($kbytes kbytes/sec, $mbmin MiB/min)";
        $options["$kbits"] = $label;
    }
...
$templateEngine = new \KotaVillage\Page();
...
function assign_vars($templateEngine)
{
	...
	$templateEngine->assign("kotavillagejsversion", $fileversions['kotavillage.js']);

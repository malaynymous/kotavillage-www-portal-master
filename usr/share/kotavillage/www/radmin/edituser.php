...
if (isset($_POST['updateusersubmit'])) {   // Process form for changed items and do updates
    ...
    // Update password
    if (\KotaVillage\Clean::text($_POST['Password']) && \KotaVillage\Clean::text($_POST['Password']) != $user['Password']) {
        DatabaseFunctions::getInstance()->setUserPassword($username, \KotaVillage\Clean::text($_POST['Password']));
    ...
    if (\KotaVillage\Clean::text($_POST['Group']) && \KotaVillage\Clean::text($_POST['Group']) != $user['Group']) {
        $temperror = validate_group($_POST['Group']);
        if (array_filter($temperror)) {
            $error = array_merge($error, $temperror);
        } else {
            DatabaseFunctions::getInstance()->setUserGroup($username, \KotaVillage\Clean::text($_POST['Group']));
    ...
    if (\KotaVillage\Clean::text($_POST['Comment']) != $user['Comment']) {
        DatabaseFunctions::getInstance()->setUserComment($username, \KotaVillage\Clean::text($_POST['Comment']));
        // TODO: Check return for success
    ...
    // Lock/Unlock update
    if (\KotaVillage\Clean::text($_POST['LockReason']) != $user['LockReason']) {
        if (\KotaVillage\Clean::text($_POST['LockReason']) == '') {
            ...
            // Using \KotaVillage\Clean::username as the LockReason is processed by JSON from CoovaChilli from Radius and so ' and " don't carry well
            DatabaseFunctions::getInstance()->lockUser($username, \KotaVillage\Clean::username($_POST['LockReason']));
            $success[] = T_("User Account Locked");
            AdminLog::getInstance()->log(
                "Account $username locked: " . \KotaVillage\Clean::username($_POST['LockReason'])
            );
        }
    }
    ...
    if ($maxMb !== ''
        ...
        if (!\kotavillage\Validate::numericLimit($maxMb)) {
    ...
    // Increase Time Limit
    if ($addTime) {
        if (!\KotaVillage\Validate::numericLimit($addTime)) {
    ...
    if ($maxTime !== ''
        ...
        if (!\KotaVillage\Validate::numericLimit($maxTime)) {

...
function validate_form($userDetails, $type = 'User')
{
    ...
    if ($type == 'Computer') {
        ...
        if (!\KotaVillage\Validate::MACAddress($userDetails['mac'])) {
            $error[] =T_("MAC Address not in correct format");
        }
    }

    if (!\KotaVillage\Validate::numericLimit($userDetails['MaxMb']) && $userDetails['MaxMb'] != '') {
        $error[] = sprintf(T_("Invalid value '%s' for 1 Data Limit"), $userDetails['MaxMb']);
    }
    if (!\KotaVillage\Validate::numericLimit($userDetails['Max_Mb']) && $userDetails['Max_Mb'] != 'inherit') {
        $error[] = sprintf(T_("Invalid value '%s' for Data Limit"), $userDetails['Max_Mb']);
    }
    if (!\KotaVillage\Validate::numericLimit($userDetails['MaxTime']) && $userDetails['MaxTime'] != '') {
        $error[] = sprintf(T_("Invalid value '%s' for Time Limit"), $userDetails['MaxTime']);
    }
    if (!\KotaVillage\Validate::numericLimit($userDetails['Max_Time']) && $userDetails['Max_Time'] != 'inherit') {
...
if (isset($_POST['newusersubmit']) || isset($_POST['newmachinesubmit'])) {
    ...
    if ($type == 'User') {
        $user['Username'] = \KotaVillage\Clean::username($_POST['Username']);
        $user['Password'] = \KotaVillage\Clean::text($_POST['Password']);
    }

    if ($type == 'Computer') {
        $user['Username'] = \KotaVillage\Clean::username($_POST['mac']);
    ...
    $user['Group'] = \KotaVillage\Clean::text($_POST['Group']);
    $user['Expiration'] = expiry_for_group(\Grase\Clean::text($_POST['Group']));
    $user['Comment'] = \KotaVillage\Clean::text($_POST['Comment']);
...
$user['Password'] = \KotaVillage\Util::randomPassword($Settings->getSetting('passwordLength'));

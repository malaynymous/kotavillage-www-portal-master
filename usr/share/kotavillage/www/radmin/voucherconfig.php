...
if(isset($_POST['submit']))
{
    ...
    foreach($vouchernames as $key => $name)
    {
        ...
        if(!\KotaVillage\Validate::numericLimit($vouchermaxtime[$key])) {
            $error[] = sprintf(T_("Invalid value '%s' for Time Limit"), $vouchermaxtime[$key]);
        }
        if(!\KotaVillage\Validate::numericLimit($vouchermaxmb[$key])) {
        ...
        $vouchersettings[\KotaVillage\Clean::groupName($name)] = array_filter(array(
            'VoucherName' => \KotaVillage\Clean::groupName($name),
            'VoucherLabel' => \KotaVillage\Clean::text($name),
            ...
            'Description' => @ \KotaVillage\Clean::text($voucherdesc[$key]),

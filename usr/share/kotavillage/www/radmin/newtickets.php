...
/*  This file is part of KOTAVILLAGE Hotspot.
    ...
    KOTAVLLAGE Hotspot is free software: you can redistribute it and/or modify
...
function validate_form()
{
    ...
    if (!\KotaVillage\Validate::numericLimit($Max_Mb)) {
    ...
    if (!\KotaVillage\Validate::numericLimit($Max_Time)) {
...
if (isset($_POST['createticketssubmit'])) {
    ...
    if ($error) {
        ...
        $group = \KotaVillage\Clean::text($_POST['Group']);
        ...
        for ($i = 0; $i < $user['numberoftickets']; $i++) {
            ...
            if ($Settings->getSetting('simpleUsername')) {
                $username = \KotaVillage\Util::randomLowercase($Settings->getSetting('usernameLength'));
            } else {
                $username = \KotaVillage\Util::randomUsername($Settings->getSetting('usernameLength'));
            }
            if ($Settings->getSetting('numericPassword')) {
                $password = \KotaVillage\Util::randomNumericPassword($Settings->getSetting('passwordLength'));
            } else {
                $password = \KotaVillage\Util::randomPassword($Settings->getSetting('passwordLength'));
            }

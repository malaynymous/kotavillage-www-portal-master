...
$wanif = array(\KotaVillage\Util::getNetworkWANIF());
$lanifs = \KotaVillage\Util::getAvailableLANIFS($wanif[0]);
...
if (isset($_POST['submit'])) {
    ...
    foreach ($singleNetworkOptions as $singleOption => $attributes) {
        switch ($attributes['type']) {
            case "string":
                $postValue = trim(\KotaVillage\Clean::text($_POST[$singleOption]));
                break;
            case "int":
    ...
    foreach ($selectNetworkOptions as $selectOption => $attributes) {
        switch ($attributes['type']) {
            case "string":
                $postValue = trim(\KotaVillage\Clean::text($_POST[$selectOption]));
    ...
    foreach ($multiNetworkOptions as $multiOption => $attributes) {
        ...
        foreach ($_POST[$multiOption] as $value) {
            switch ($attributes['type']) {
                case "string":
                    $postValue[] = \KotaVillage\Clean::text($value);
                    break;
                case "int":

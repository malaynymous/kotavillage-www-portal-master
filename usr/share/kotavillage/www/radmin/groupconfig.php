...
if (isset($_POST['submit'])) {
    ...
    foreach ($groupNames as $key => $name) {
        ...
        if(!\KotaVillage\Validate::numericLimit($groupTimeLimit[$key])) {
            $error[] = sprintf(T_("Invalid value '%s' for Time Limit"), $groupTimeLimit[$key]);
        }
        if(!\KotaVillage\Validate::numericLimit($groupRecurTimeLimit[$key])) {
            $error[] = sprintf(T_("Invalid value '%s' for Time Limit"), $groupRecurTimeLimit[$key]);
        }
        if(!\KotaVillage\Validate::numericLimit($groupRecurDataLimit[$key])) {
            $error[] = sprintf(T_("Invalid value '%s' for Data Limit"), $groupRecurDataLimit[$key]);
        }
        if(!\KotaVillage\Validate::numericLimit($groupIdleTimeout[$key])) {
            $error[] = sprintf(T_("Invalid value '%s' for Idle Timeout"), $groupIdleTimeout[$key]);
        }
        if(!\KotaVillage\Validate::recurrenceInterval($groupRecurTime[$key], recurtimes())) {
            $error[] = sprintf(T_("Invalid recurrence interval '%s'"), $groupRecurTime[$key]);
        }
        if(!\KotaVillage\Validate::recurrenceInterval($groupRecurData[$key], recurtimes())) {
            $error[] = sprintf(T_("Invalid recurrence interval '%s'"), $groupRecurData[$key]);
        }
        if(!\KotaVillage\Validate::recurrenceTime($groupRecurTime[$key], $groupRecurTimeLimit[$key])) {
            $error[] = T_("Recurring time limit must be less than interval");
        }
        if(!\KotaVillage\Validate::bandwidthOptions($groupBandwidthDownLimit[$key], bandwidth_options())) {
            $error[] = sprintf(T_("Invalid Bandwidth Limit '%s'"), $groupBandwidthDownLimit[$key]);
        }
        if(!\KotaVillage\Validate::bandwidthOptions($groupBandwidthUpLimit[$key], bandwidth_options())) {
        ...
        $groups[\KotaVillage\Clean::groupName($name)] = array_filter(
            array(
                'DataRecurTime' => \KotaVillage\Clean::text($groupRecurData[$key]),
                'DataRecurLimit' => clean_number($groupRecurDataLimit[$key]),
                'TimeRecurTime' => @ \KotaVillage\Clean::text($groupRecurTime[$key]),
        ...
        $groupSettings[\KotaVillage\Clean::groupName($name)] = array_filter(
            array(
                'GroupName' => \KotaVillage\Clean::groupName($name),
                'Comment' => \KotaVillage\Clean::text($groupComment[$key]),
                'GroupLabel' => \KotaVillage\Clean::text($name),

...
if (isset($_GET['user'])) {
    $users = $DBF->getMultipleUsersDetails(array(\KotaVillage\Clean::text($_GET['user'])));
    if (!is_array($users)) {
        $users = array();
    }
    $title = \KotaVillage\Clean::text($_GET['user']) . ' Voucher';
    ...
    foreach ($groups as $group) {
        $group = \KotaVillage\Clean::groupName($group);
